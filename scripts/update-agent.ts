// ONE-TIME UPDATE — pushes new system prompt to existing agent
...
async function updateAgent() {
  const { AGENT_ID } = JSON.parse(
    readFileSync(".agent-config.json", "utf-8")
  );

  // Fetch current agent to get its version (required by the update endpoint)
  const current = await client.beta.agents.retrieve(AGENT_ID);
  const version = (current as any).version;
  console.log(`Current agent version: ${version}`);

  console.log(`Updating agent ${AGENT_ID} with Voice Anchor system prompt...`);
  ...
  console.log(`Done. Agent version: ${(updated as any).version ?? "n/a"}`);
  console.log("Voice Anchor is now live.");
}
